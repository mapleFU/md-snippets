...
impl ChunkedVec {
    pub fn reserve(&mut self, sz: usize) {
        match self {
            ChunkedVec::I32Chunk(v) => v.data.reserve(sz),
            ChunkedVec::I64Chunk(v) => v.data.reserve(sz),
            ChunkedVec::BytesChunk(v) => v.data.reserve(sz),
        }
    }
}
...
pub struct ChunkedSortKeys {
    pub data: Vec<ChunkedVec>,
}

pub struct NonChunkedSortKeys {
    pub data: Vec<NonChunkedVec>,
}

impl NonChunkedSortKeys {
    pub fn new() -> Self {
        NonChunkedSortKeys { data: vec![] }
    }

    pub fn reserve(&mut self, sz: usize) {
        self.data.reserve(sz);
    }
}
...
impl ChunkedSortKeys {
    pub fn compare_idx(&self, idx: usize, other: &NonChunkedVec) -> Option<Ordering> {
        for current_index_idx in 0..self.data.len() {
            let result = match (
...
impl SortKeys for ChunkedSortKeys {
    ...
    fn upper_bound(&self, _d: &NonChunkedVec) -> usize {
        todo!()
    }
}
